...
CREATE TABLE IF NOT EXISTS `lab_mysql`.`Cars` (
  `Manufacter` VARCHAR(50) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `lab_mysql`.`Customers` (
  ...
  `Customerzip` INT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `lab_mysql`.`Invoices` (
  ...
  PRIMARY KEY (`InvoiceID`, `Salespersons_StaffID`, `Customers_CustomerID`, `Cars_CarID`),
  ...
  CONSTRAINT `fk_Invoices_Cars1`
    ...
    REFERENCES `lab_mysql`.`Cars` (`CarID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
